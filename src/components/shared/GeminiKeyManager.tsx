...
const STORAGE_KEY = "vanity_gemini_key"
...
export function GeminiKeyManager() {
  const { key, saveKey, removeKey } = useGeminiKey()
    ...
    return (
      <div className="flex items-center gap-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold truncate">Gemini API Key active</p>
  ...
  return (
    <div className="space-y-4 p-6 rounded-2xl bg-white/5 border border-white/10 animate-in fade-in duration-500">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h3 className="text-sm font-bold font-syne">Gemini Vision Activation</h3>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Enter your Gemini API key to enable AI mode. Stored locally only; requests are sent directly from your browser.
          </p>
        </div>
      </div>
      ...
      <div className="text-[10px] text-center text-muted-foreground/50">
        <p>Get a free key from <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-emerald-500 transition-colors">Google AI Studio</a></p>
      </div>
    </div>
  )
}

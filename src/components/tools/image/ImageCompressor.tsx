...
import { Download, Minimize2, CheckCircle, AlertCircle, FileArchive, Loader2, Info, ArrowLeft, Layers, Gauge } from "lucide-react"
      ...
      <div className="space-y-8">
        {!file && queue.length === 0 ? (
          <div className="space-y-8 animate-in fade-in duration-500">
             ...
             <DropZone onDrop={handleFiles} multiple={processMode === 'batch'} label={processMode === 'batch' ? "Drop multiple images for batch compression" : "Drop image to compress"} />
          </div>
        ) : (
                ...
                <div className="space-y-6">
                   ...
                   <div className="flex justify-center">
                      <button 
                        onClick={async () => {
                           const zip = new JSZip()
                           queue.filter(i => i.resultBlob).forEach(i => zip.file(i.file.name.split(".")[0] + ".jpg", i.resultBlob!))
                           const blob = await zip.generateAsync({ type: "blob" })
                           downloadBlob(blob, `vanity-compressed-batch.zip`)
                        }}
                      ...
                      >
                         <FileArchive className="w-6 h-6" /> Download ZIP ({queue.filter(i => i.resultBlob).length})
                      </button>
                   </div>
                </div>
             ) : (

...
import { Eye, Layout, Copy, CheckCircle, Download, BookOpen, Sparkles, Plus, Search } from "lucide-react"
...
ProductName is a **next-generation SaaS platform** that helps teams collaborate and ship faster.
...
**Game Title** is an action-adventure game. Explore procedurally generated worlds, battle enemies, and uncover ancient mysteries.
...
# Open in Godot 4.0+ and press F5
\`\`\`
...
<p align="center">Built with ❤️ using Godot Engine</p>`
  }
]
...
export function MarkdownPreview() {
  ...
  const [activeTab, setActiveTab] = useState<"edit" | "badges" | "templates">("edit")
  const [badgeSearch, setBadgeSearch] = useState("")
  const [activeBadgeCat, setActiveBadgeCat] = useState("frontend")
  const [previewTheme, setPreviewTheme] = useState<"vanity" | "github">("vanity")
  ...
  return (
    <ToolLayout
      ...
      description="Write markdown, get beautiful live rendering. Includes badge gallery and README templates."
      icon={Eye}
      maxWidth="max-w-7xl"
      centered={true}
    >
        ...
        <div className="lg:col-span-5 flex flex-col space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleCopyHtml} className="px-3 py-1.5 bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest rounded-lg flex items-center gap-2 hover:bg-white/10 transition-all">
                {copied ? <CheckCircle className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                {copied ? "Copied" : "HTML"}
              </button>
              <button onClick={handleDownloadMd} className="px-3 py-1.5 bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-widest rounded-lg flex items-center gap-2 hover:scale-105 active:scale-95 transition-all">
          ...
          {activeTab === "badges" && (
            <div className="flex-1 min-h-[60vh] bg-black/40 border border-white/10 rounded-2xl p-6 space-y-5 overflow-y-auto custom-scrollbar">
              <div className="relative">
                ...
                <input type="text" placeholder="Search badges..." value={badgeSearch} onChange={(e) => setBadgeSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white outline-none focus:border-primary/50 transition-all" />
              </div>

              {!badgeSearch && (
                <div className="flex gap-1.5 flex-wrap">
                  {Object.entries(BADGE_CATEGORIES).map(([key, cat]) => (
                    <button key={key} onClick={() => setActiveBadgeCat(key)} className={cn("px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all border", activeBadgeCat === key ? "bg-primary/20 border-primary text-primary" : "bg-white/5 border-white/5 text-muted-foreground hover:bg-white/10")}>
                      {cat.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {filteredBadges.map(badge => (
                  <button key={badge.name} onClick={() => insertBadge(badge)} className="flex items-center gap-3 p-3 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 hover:border-white/20 transition-all group">
          ...
          {activeTab === "templates" && (
            <div className="flex-1 min-h-[60vh] bg-black/40 border border-white/10 rounded-2xl p-6 space-y-4 overflow-y-auto custom-scrollbar">
              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Quick-start templates</p>
              <div className="space-y-3">
                {TEMPLATES.map(t => (
                  <button key={t.id} onClick={() => loadTemplate(t.id)} className="w-full text-left p-5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-primary/30 transition-all group">
                    <p className="text-sm font-bold text-white group-hover:text-primary transition-colors">{t.name}</p>
                    <p className="text-[10px] text-muted-foreground mt-1">Click to load template and switch to editor</p>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-7 flex flex-col space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1.5">
              <button onClick={() => setPreviewTheme("vanity")} className={cn("px-2 py-1 text-[9px] font-bold uppercase tracking-widest rounded-md transition-all", previewTheme === "vanity" ? "bg-primary/20 text-primary" : "text-muted-foreground hover:text-white")}>Vanity</button>
              <button onClick={() => setPreviewTheme("github")} className={cn("px-2 py-1 text-[9px] font-bold uppercase tracking-widest rounded-md transition-all", previewTheme === "github" ? "bg-primary/20 text-primary" : "text-muted-foreground hover:text-white")}>GitHub</button>
            </div>
          </div>
          ...
          {previewTheme === "github" ? (
            <div className="flex-1 min-h-[60vh] bg-[#0d1117] border border-[#30363d] rounded-2xl overflow-auto custom-scrollbar">
              <div className="flex items-center gap-1 px-4 py-3 border-b border-[#30363d] bg-[#161b22]">
                <div className="px-4 py-1.5 text-xs font-semibold text-white bg-[#0d1117] border border-[#30363d] border-b-[#0d1117] rounded-t-md -mb-[13px]">README.md</div>
              </div>
              <div className="p-8 md:p-12 readme-github-theme">
            ...
            <div className="flex-1 min-h-[60vh] bg-white/[0.03] border border-white/5 rounded-2xl p-8 overflow-auto prose prose-invert prose-emerald max-w-none shadow-inner custom-scrollbar">
              <div dangerouslySetInnerHTML={{ __html: html }} />
              {input === "" && (
      ...
      <style>{`
        ...
        .readme-github-theme { color: #e6edf3; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Noto Sans', Helvetica, Arial, sans-serif; }
        ...
        .readme-github-theme blockquote { border-left: 4px solid #30363d; color: #8b949e; padding: 0 1em; }
        .readme-github-theme table { border-collapse: collapse; }

...
export function CompressPdf() {
  ...
  useEffect(() => {
    if (file && isTextHeavy && ratio <= 0.6 && mode === "smart") {
      setMode("extreme")
    }
  }, [ratio, isTextHeavy, mode, file])
  ...
  const runSmartCompress = async (file: File) => {
    ...
    // Fallback detection
    if (result.size / 1024 > targetSizeKB + 50 && ratio < 0.8) {
  ...
  const runExtremeCompress = async (file: File, targetKB: number) => {
    ...
    if (targetKB < absoluteMin) {
      throw new Error(`Target too low. Minimum possible for this document is ~${Math.ceil(absoluteMin)}KB.`)
    }
    ...
    while (iteration++ < MAX_ITERS) {
      ...
      for (let i = 1; i <= pageCount; i += concurrency) {
        setPhase(`Rendering (Page ${i}/${pageCount})...`)
        const chunk = Array.from({ length: Math.min(concurrency, pageCount - i + 1) }, (_, idx) => i + idx)
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold font-syne">Compress PDF</h1>
           ...
           <div className="glass-panel p-6 rounded-xl space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button 
                  ...
                  disabled={isTextHeavy && ratio < 0.6}
                  className={cn(
                    ...
                    isTextHeavy && ratio < 0.6 && "cursor-not-allowed grayscale"
                  )}
                >
                  Smart
                </button>
